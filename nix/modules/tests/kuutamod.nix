...
makeTest' {
  ...
  testScript = ''
    ...
    # make sure prometheus exporter has latest state
    server.succeed("systemctl restart prometheus-consul-exporter.service")

    server.wait_for_unit("prometheus-consul-exporter.service")
    ...
    # consul exporter endpoint
    out = server.succeed("curl --silent http://127.0.0.1:9107/metrics")
    print(out)
    assert "consul_up 1" in out, "consul is not up"
